...
const secureCredentials = require('./secureCredentialsManager');
...
class FractalBitcoinService {
    constructor() {
        ...
        // Initialize secure house wallet credentials
        this.houseWallet = {
            address: '',
            privateKey: '',
            publicKey: ''
        ...
        this.fractalApiBreaker = circuitBreakerService.getFractalApiBreaker();
        this.indexerBreaker = circuitBreakerService.getExternalApiBreaker('fractal-indexer');
        this.explorerBreaker = circuitBreakerService.getExternalApiBreaker('fractal-explorer');
        
        // Initialize credentials securely
        this.initPromise = this.initializeSecureCredentials();
        
        console.log(`🔗 FractalBitcoinService initialized for ${this.networkType}`);
    }

    // Initialize credentials from secure storage
    async initializeSecureCredentials() {
        try {
            // Load house wallet credentials securely
            const houseWalletCredentials = await secureCredentials.getHouseWalletCredentials();
            
            this.houseWallet = {
                address: houseWalletCredentials.address,
                privateKey: houseWalletCredentials.privateKey,
                publicKey: houseWalletCredentials.publicKey
            };

            // Load API key securely
            try {
                this.apiKey = await secureCredentials.getCredential('fractalApiKey');
            } catch (error) {
                console.warn('⚠️  Fractal API key not found in secure storage, using environment variable');
                this.apiKey = process.env.FRACTAL_API_KEY || '';
            }

            console.log('🔐 House wallet credentials loaded securely');
            console.log(`🏦 House wallet address: ${this.houseWallet.address.substring(0, 10)}...`);
            
            return true;
        } catch (error) {
            console.error('❌ Failed to load secure credentials:', error.message);
            console.warn('⚠️  Falling back to environment variables (INSECURE)');
